...
rscs_e rscs_bmp280_calculate(const rscs_bmp280_calibration_values_t * calvals , int32_t rawpress, int32_t rawtemp, float * press_p, float * temp_p) {
...
int32_t t_fine;
...

	{
		...
		if (var1_p == 0)	return RSCS_E_NULL; // чтобы не делить на ноль
