...
typedef enum
{
	RSCS_UART_FLAG_ENABLE_TX = 0x01,	// использовать TX для этого уарта 0001
	RSCS_UART_FLAG_ENABLE_RX = 0x02,	// использовать RX для этого уарта 0010
#ifdef RSCS_UART_USEBUFFERS
	RSCS_UART_FLAG_BUFFER_TX = 0x04,	// использовать буферизацию на TX для этого уарта 0100
	RSCS_UART_FLAG_BUFFER_RX = 0x08,	// использовать буферизацию на RX для этого уарта 1000
#endif
} rscs_uart_mode_t;
